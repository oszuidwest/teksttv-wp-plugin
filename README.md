...
Loop and ticker changes invalidate only their channel. Each supported option or term-meta mutation deletes its affected transient, so a cache rebuilt between two mutations is invalidated again. Standalone post term assignments are flushed once at the end-of-request boundary; a full post save consumes that pending change and invalidates once after all terms and meta are stored.
